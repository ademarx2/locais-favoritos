...
const PlaceFormScreen: React.FC = () => {
  ...
  }, [placeId, route.params]);

  const handleSave = () => {
    const lat = parseFloat(latitude);
    const lon = parseFloat(longitude);

    if (!name || isNaN(lat) || isNaN(lon) || !color) {
      Alert.alert('Erro', 'Por favor, preencha todos os campos corretamente.');
      return;
    }

    if (placeId) {
      placesContext.updatePlace({ id: placeId, name, latitude: lat, longitude: lon, color });
    } else {
      placesContext.addPlace({ name, latitude: lat, longitude: lon, color });
    }
    navigation.goBack();
  };
      ...
      Alert.alert(
        ...
        [
          ...
          { text: 'Apagar', style: 'destructive', onPress: () => {
            placesContext.removePlace(placeId);
            navigation.goBack();
          }},
        ]
      );
  ...
  return (
    <Container>
      ...
      <Input value={color} onChangeText={setColor} placeholder="Ex: #FF0000" />
